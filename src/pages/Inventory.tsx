...
export function BarcodeScanner({ open, onOpenChange, onScanned, title = "Scan Barcode" }: BarcodeScannerProps) {
  ...
  const decodeImageFile = async (file: File) => {
    ...
    try {
      ...
      // Fallback: ZXing — try multiple canvas scales for best decode chance
      ...
      // Try decoding at multiple scales — ZXing works best at certain resolutions
      ...
      for (const scale of scales) {
        try {
          ...
          const result = await zxing.decodeFromImageUrl(scaledDataUrl);
